...
<title>FELIZIA – Inter Campus Fest</title>
...
<header id="main-header" style="display:none">
  <div class="brand">
    <img id="header-logo" class="logo-circle" src="" alt="F" onerror="this.src=''; this.alt='F'; this.style.backgroundColor='var(--primary)'" style="width:40px; height:40px;">
    <div>
      <h3 style="margin:0; font-size:20px; color:var(--primary-dark)">FELIZIA</h3>
      <span style="font-size:12px; color:var(--text-muted)">Inter Campus Fest</span>
    </div>
  </div>
  <div class="row">
...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
const Store = {
  key: "alif_fest_data_v5_felizia_final", // Updated version key
  
  defaults: () => ({
    meta: { version: 5 },
    admin: { username: 'admin', password: 'felizia' }, 
    teams: [
    ...
    // Chest Card Config (Unchanged structure)
    chestConfig: {
        ...
        elements: {
            ...
            fest: { x: 50, y: 10, size: 18, color: "#0056b3", bold: true, text:"FELIZIA Fest", visible: true }
        }
    }
  }),
  ...
  load() {
    ...
    if (!d && localStorage.getItem("alif_fest_data_v4_felizia")) {
        ...
        console.warn("Migrating data from v4 to v5 (Felizia final).");
        return data; 
    }
  ...
  exportJSON(data) {
    ...
    a.href = url; a.download = `FELIZIA_Data_${new Date().toISOString().slice(0,10)}.json`;
    a.click();
  },
...
const App = {
      ...
      <div class="login-wrapper">
        <div class="card login-box">
          <div style="text-align:center; margin-bottom:25px;">
            <img id="login-logo" class="logo-circle" src="" alt="F" onerror="this.src=''; this.alt='F'; this.style.backgroundColor='var(--primary)'" style="margin:0 auto 10px auto; width:50px; height:50px;">
            <h1>FELIZIA Fest Portal</h1>
            <p>Inter Campus Literary Arts Fest</p>
          </div>
  ...
  renderTabContent() {
    ...
    if(this.state.role === 'admin') {
        ...
        if(t === 'students_report') this.renderAdminStudentsReport(el); // New
        if(t === 'entries_report') this.renderAdminEntriesReport(el); // New
        ...
        if(t === 'comp_posters') this.renderAdminCompPosters(el); // New
  ...
  printReport(id) {
    ...
    pArea.innerHTML = `<div style="padding:40px; text-align:center; font-family:var(--font-heading); background:#fff;">
      <h1 style="font-size:30px; color:var(--primary-dark)">FELIZIA Fest - ${title}</h1>
  ...
  /* --------------------------------------------------------------------------
     ADMIN MODULES - COMPETITION POSTER (New)
     -------------------------------------------------------------------------- */
